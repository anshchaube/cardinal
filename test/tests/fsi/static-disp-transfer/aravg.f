...
c-----------------------------------------------------------------------
      real function emap_max(qty,emap)
      ...
      do e=1,nelt
       if (emap(e).gt.0) then
         ...
         do j1 = js1,jf1,jskip1
           qpt = qty(j1,j2,1,e)
           if(qpt.gt.qtymax) qtymax = qpt
         enddo
         enddo
       endif
      enddo

      emap_max = glmax(qtymax,1)

      return
      end
c-----------------------------------------------------------------------
      real function emap_min(qty,emap)
      ...
      do e=1,nelt
       if (emap(e).gt.0) then
         ...
         do j1 = js1,jf1,jskip1
           qpt = qty(j1,j2,1,e)
           if(qpt.lt.qtymin) qtymin = qpt
         enddo
         enddo
       endif
      enddo

      emap_min = glmin(qtymin,1)
...
C-----------------------------------------------------------------------
      subroutine check_sidesets()
c     checks sidesets by dumping sideset ID in the x-velocity field
c     check output files titled "bid<casename>" in paraview
c     use the extract selection filter
      implicit none
      include 'SIZE'
      include 'TOTAL'
      integer e,f,bid,js1,jf1,jskip1,js2,jf2,jskip2,j1,j2
      ...
      call outpost(vx,vy,vz,pr,t,'bid')
